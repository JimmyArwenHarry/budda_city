...
function summaryInstruction(style?: StoryStyle, firstGiant?: GiantKey): string {
  const styleNote = style
    ? `\n- 文风须贴合本局风格（${styleLabel(style)}），把六巨头与结局按该风格的设定来总结。`
    : "";
  const giantNote = firstGiant
    ? `\n- 本局测字缘主是「${giantLabel(firstGiant)}」，总结须突出玩家与这位巨头从初遇到终局的缘分线。`
    : "";
...
- 总结要有画面感与节奏感，读起来像一段佛城说书。${styleNote}${giantNote}
直接输出总结正文即可，不要JSON，不要markdown标题，不要"总结如下"之类的多余前缀。`;
}

export async function POST(req: NextRequest) {
  let body: { history?: ChatMessage[]; style?: unknown; firstGiant?: unknown };
  try {
    body = await req.json();
  } catch {
  ...
  const messages: ChatMessage[] = [
    ...
    ...history,
    { role: "user", content: summaryInstruction(style, firstGiant) },
  ];
