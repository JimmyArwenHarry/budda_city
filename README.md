...
本站内置两个独立剧本，首页是赛博玄幻的**佛城**，右下角（或直接访问 `/jinyong`）可进入金庸文风的**江湖**。

| 剧本 | 路径 | 题材 | 文风 |
| --- | --- | --- | --- |
| 佛城风云：六巨头列传 | `/` | 赛博玄幻 · 黑色幽默 | 佛城黑话 |
| 江湖录：卑贱子的逆命 | `/jinyong` | 乱世武侠 · 小人物逆命 | 金庸笔法 |
...
金庸文风的文字武侠 RPG，与佛城共用同一套 DeepSeek 调用与可靠性约束，独立成剧。

## ✨ 特性

- **择一身世**：农民🌾 / 乞丐🥣 / 小流氓🔪 / 妓女🌺 / 落魄书生📜 / 更夫🏮，皆乱世蝼蚁
- **卦摊测字**：写下一个字，算命先生按笔画、五行、字义与意境判你一桩「命格」，贯穿全剧并在终局点题
- **10 回江湖**：金庸笔法实时写就（半文半白、写意武功、恩怨情仇）
- **终局三段式**：第 9 回大高潮 → 第 10 回结局 → **约 800 字金庸文风战记 + 一首七律**（八句七言、对仗押韵）
- 墨 / 朱砂 / 鎏金主题，与佛城的赛博荧光绿区分

## 玩法

1. 首页右下角点「🏯 另辟 · 金庸江湖」（或直接访问 `/jinyong`）
2. 择一身世 → 卦摊写下一个字，测得命格 → 踏入江湖
3. 10 回抉择，第 10 回结算命运
4. 生成约 800 字战记与一首七律，可抄录分享

## 代码结构（与佛城平行）

```
app/jinyong/page.tsx            # 江湖入口页
app/api/jinyong/{divination,chat,summary}/route.ts
components/jinyong/             # JinYongGame / StartScreen / FortuneScreen / EndingView / Loading
lib/jinyong.ts                  # 身份/命格数据 + callFortune + callChronicle（800字战记 + 七律）
lib/jinyong-prompt.ts           # 金庸文风基底提示词 + 身份/命格指令置顶
```

> 复用 `lib/deepseek.ts` 的 `singleCall`（已导出），同样的三条铁律：USER 收尾、不用 `response_format: json_object`、结构化正文契约 + 服务端容错解析。
