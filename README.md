...
一款基于 **Next.js + DeepSeek API** 的 13 回合文字 RPG 游戏。
...
- **13 回合回合制**：顶部回合指示器 + 流光进度条（`Turn 3/13`）
- **AI 实时剧情**：每回合调用 DeepSeek 生成 100~200 字剧情 + 3 个恶搞选项
- **打字机效果**：剧情逐字浮现，点击剧情卡片可跳过
- **终局三段式**：第 12 回合大高潮 → 第 13 回合结算命运 → 生成约 800 字《佛城战记》剧情总结
...
1. 开局先选择一种**剧情风格**（武侠/玄幻/科幻/商战/校园/末世），再点击 **踏入佛城 ▶** 开始
2. 每回合阅读 AI 生成的剧情，选择 3 个选项之一
3. 你的选择会写入对话历史，影响后续剧情与结局走向
4. **第 12 回合**进入六巨头大混战高潮
5. **第 13 回合**结算你的最终命运
6. AI 接着根据完整剧情生成约 800 字《佛城战记》总结
7. 再根据战记总结为你颁发 3 个专属成就
8. 复制战记与成就分享给朋友，或重新开始
